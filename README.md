Holidays For You is a holiday platform designed to help users find a spot of interest near them or near to where they're going. This can all be achieved easily through the use of the 
...
been made in mind to accommodate these changes, by having buttons in place to link to these places, as well as a common theme in mind to use across the pages. Bootstrap has also been used 
...
down to the map positioned near the bottom of the page. The reason for this is so users must pass by promotions on bigger screens, and more importantly the instructions on how to use the API before 
...
The process of getting to the map is streamlined and easy, no matter what screen size the user is on the instructions for the map are always visible allowing the user to always know they are in 
the right place and have not become lost. 

In terms of the theme of the page I have used a sky blue in collaboration with the plane logo to represent the travel that the users will take. On top of this black has been chosen as the 
colour of the typeface to make it stand out, allowing users to easily read the text. along with this the Montserrat Alternates font has been used as this provides an easy to read and 
aesthetically pleasing appeal. Another aspect of the header is the nav bar which is left simple making use of drop downs allowing for main categories and subcategories of linked pages, as 
well as on smaller screens the entirety of the navbar becomes a drop. This is to avoid it taking up too much screen real estate. The final aspect is that the header has a shadow effect this is 
to create the illusion of depth, as well as the coordinate with the place logo giving the effect of flight.

Straight after the header is a row of promotional cards. These are formatted showing an image of the destination followed by a brief description of the area. This is so the user can at glance get 
information on these promotions without paying much attention to them. On a larger screen this format works very well. However, on a screen with less space these would take up too much space. 
...
to get information at a glance despite this the eye catching images should still entice the user to click on a card and learn more from there. Third the cards disappear altogether leaving 
...
For the API map the styling has been left how the creator intended them this is because the formatting is the same as that as Google Maps. So users that are familiar with that platform will 
...
* As a general user, "I can look around the map but that's about it. I can't click on anything else. But I'd like to go to Rome please." 
...
To help avoid this in the future the instructions will be written more clearly to help diminish the likelihood of this happening. 

* As an admin user, "I wanted to test to the Google API Maps and Places it took a few attempts to get the result I wanted" 
After some testing it is clear the API requires a specific structure so that the user can get the intended result. To combat this the instructions will be rewritten to reflect this.
...
* Navbar 
    * The navbar is positioned at the very top of the page; this is an industry standard and there's no reason to reinvent the wheel. However, the navbar contains a few tricks to assist in UX. 
    ...
    down this is partly for styling purposes, as well as taking up less of the screen. The last aspect is the name of the site disappears only leaving the logo and drop down. Originally this felt 
    unintuitive but on smaller screens there is no row of promotions before the "welcome" text so the website starts off with a "Welcome to Holidays For You!" message, still saying the name of 
    the company.

* Promotion Cards 
    * The promotion cards' purpose is all in the name they promote potential holidays to any visitors to the site. What isn't in the name is where they are positioned, after the navbar this is 
    ...
    some time to get past the advertisement. This is not the purpose of the quick glance approach they are meant for. So with this in mind the cards first go from four to three and later lose their 
    ...
    screens as there is not enough space to accommodate the feature. 
...
* Instructions for the API usage
    * This is a very small but important section of the page. The steps are written in a non-formal manner to help relax users who go to use the API. As some users will not have experience 
    with applications such as this one. The steps are separated into their own lines; this is so the user reads them as steps without making it obvious that they are. Again this is all down to 
    relax an inexperienced user to the thought of using something they are unfamiliar with, as this can be a daunting task.  
    ...
    * The APIs used are the Google Maps and Places APIs supplied by Google. 
        1. Maps
            * This allows for the website to display dynamic and static maps of real world places, as well as allow them a street view 360&deg;. This is very helpful as users can search anywhere 
            in the world and be taken to it. This required very little effort on my part, other than reading through the documentation and understand the implementation process everything else 
        ...
        2. Places
            * This API is the backbone of the website whereas maps allows a user to see anywhere the places API allows the user to see what places of interest there are in an area. The API itself 
            can be a bit finicky so instructions has been written to help users when using the API. Despite some small shortcomings the Google Places API works very well for the purposes required.
...
* Covid 19 travel Warning 
    * With the world in its current state it would be irresponsible to not warn users of the potential risks of traveling at the present time. The reason for this warning being positioned so 
    closely to the holiday links is so that users see it before clicking on a "type of holiday link". The title of the section is also in bold for the purposes of catching the user’s eye. 

### Features Left to Implement

* Navbar Links 
    * Currently the project is just a single webpage that can easily be expended upon by creating additional pages and linking them through the navbar. 

* Promotion Cards Pages and Links
    * The promotion cards still need information pages about them to help inform users about what is being offered. On top of this links would be needed to link these pages together. The benefit 
    of this is that these holidays can be advertised to the user properly, rather than just being eye catching and then scrolled past.

* Holiday types Pages and Links
    * At the moment the holiday types are just for decoration to expand upon this multiple pages need to be made and then linked to. This would create a more dynamic experience for users, 
    as there would be more for them to experience and explore. 
...
2. Promotion Cards
    ...
    * ii. Go to breakpoint 4 turn to 3 Cards
